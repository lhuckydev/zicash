...
const CountdownTimer = () => {
  ...
  return (
    <div className="flex items-center gap-1.5 text-white font-bold text-[10px] md:text-sm">
      <span className="opacity-80 font-medium hidden sm:inline">Ends In:</span>
...
function FlashSaleCard({ product }: { product: Product }) {
  ...
  const stockDisplay = Math.min(currentStock, 49); 
    ...
    <Link href={`/product/${product.id}`} className="block shrink-0 w-[140px] md:w-[180px] group">
      <div className="bg-white rounded-xl p-2 space-y-2">
        <div className="relative aspect-square rounded-lg bg-slate-50 overflow-hidden">
          ...
          />
          {discountPercent > 0 && (
            <div className="absolute top-1 right-1 bg-red-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded shadow-lg">
              -{discountPercent}%
            </div>
          )}
        </div>
        ...
        <div className="space-y-1">
          <h4 className="text-[10px] md:text-xs font-bold text-slate-700 line-clamp-1 truncate uppercase">
            {product.name}
          </h4>
          <div className="space-y-0">
            <p className="text-xs md:text-base font-black text-red-600 leading-none italic">
  ...
  return (
    <section className="px-1 md:px-0">
      <div className="bg-red-600 rounded-2xl overflow-hidden shadow-xl shadow-red-900/10 border-2 border-red-600">
        <div className="px-4 py-3 md:px-6 md:py-4 flex items-center justify-between bg-gradient-to-r from-red-600 to-red-700">
          ...
          <Link href="/flash-sales" className="flex items-center gap-1 text-white/90 hover:text-white text-[10px] font-black uppercase tracking-widest transition-colors">
        ...
        <div className="bg-white p-3 md:p-4">
          {isLoading ? (
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              ...
              <Link href="/flash-sales" className="shrink-0 w-24 md:w-32 flex flex-col items-center justify-center gap-2 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors border-2 border-dashed border-slate-200">
