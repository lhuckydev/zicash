...
export default function AdminPage() {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      // Re-verify session to ensure token is valid for write
      const { data: { session } } = await supabase.auth.getSession();
  ...
  const SidebarItem = ({ tab, icon: Icon, active, onClick, children }: any) => (
    <button onClick={onClick} className={cn("flex items-center justify-between w-full px-4 py-2.5 rounded-lg transition-all text-sm font-medium", active ? "bg-blue-600 text-white" : "text-white/50 hover:bg-white/5 hover:text-white")}>
  ...
  const SubItem = ({ tab, active, onClick }: any) => (
    <button onClick={onClick} className={cn("flex items-center gap-3 w-full pl-11 py-2 rounded-lg text-xs font-medium transition-all", active ? "text-white" : "text-white/40 hover:text-white")}>
      ...
      <aside className="hidden lg:flex w-64 bg-slate-950 flex-col shadow-xl z-50 shrink-0 text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-8 scrollbar-hide">
          <nav className="space-y-1">
            <SidebarItem tab="Dashboard" icon={LayoutDashboard} active={activeTab === "Dashboard"} onClick={() => setActiveTab("Dashboard")} />
            <div className="space-y-1">
              ...
              {isEcommerceOpen && (
                <div className="space-y-1">
                  <SubItem tab="Orders" active={activeTab === "Orders"} onClick={() => setActiveTab("Orders")} />
                  <SubItem tab="Products" active={activeTab === "Products"} onClick={() => setActiveTab("Products")} />
                  <SubItem tab="Invoices" active={activeTab === "Invoices"} onClick={() => setActiveTab("Invoices")} />
                </div>
              )}
            </div>
            <SidebarItem tab="Customers" icon={Users} active={activeTab === "Customers"} onClick={() => setActiveTab("Customers")} />
            <SidebarItem tab="Analytics" icon={TrendingUp} active={activeTab === "Analytics"} onClick={() => setActiveTab("Analytics")} />
            <SidebarItem tab="Calendar" icon={CalendarIcon} active={activeTab === "Calendar"} onClick={() => setActiveTab("Calendar")} />
            <SidebarItem tab="Tasks" icon={CheckSquare} active={activeTab === "Tasks"} onClick={() => setActiveTab("Tasks")} />
          </nav>
        </div>
        <div className="p-4 border-t border-white/5 space-y-1">
          <SidebarItem tab="Settings" icon={Settings} active={activeTab === "Settings"} onClick={() => setActiveTab("Settings")} />
          <button 
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 sticky top-0 z-40">
          <div className="flex items-center gap-6 flex-1">
            <Button variant="ghost" size="icon" className="lg:hidden text-slate-500"><Menu className="w-5 h-5" /></Button>
